...
httpServices.defaultUrl = function (path) {
    ...
    var url = 'https://sheets.googleapis.com/v4/spreadsheets/' + sheet.id + '/values/' + sheet.tabName + path;
    return url;
}
...
httpServices.get = function (successCallback, errorCallback) {
    ...
    var url = httpServices.defaultUrl('!A:F');
    if (!url) {
        handleNotSheetData();
        return;
    }
...
httpServices.post = function (jsonData, successCallback, errorCallback) {
    ...
    var url = httpServices.defaultUrl('!A1:F1:append?valueInputOption=USER_ENTERED');
    if (!url) {
        handleNotSheetData();
        return;
    }
